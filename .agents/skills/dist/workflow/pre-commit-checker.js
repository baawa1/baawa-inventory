        ...
        for (const file of files) {
            ...
            try {
                ...
                if (hasExport && !content.includes('withAuth') && !content.includes('withPermission')) {
                    ...
                    const isPublic = ['auth/register', 'auth/login', 'auth/verify', 'health'].some(p => relativePath.includes(p));
                    if (!isPublic) {
                        issues.push({
                            severity: 'warning',
                            riskLevel: 'high',
...
//# sourceMappingURL=pre-commit-checker.js.map