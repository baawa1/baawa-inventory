...
export class AuditLogMonitor {
  ...
  private auditableOperations: AuditableOperation[] = [
    {
      category: 'Financial',
   ...
   */
  async monitor(): Promise<SkillResult<AuditReport>> {
    try {
   ...
   */
  private async validateAuditLogger(): Promise<boolean> {
   ...
   */
  private async checkApiRoutes(): Promise<SecurityIssue[]> {
    ...
    for (const file of apiFiles) {
      ...
      if (sensitiveOps.length > 0) {
        ...
        if (!hasAuditLogging) {
          ...
          const missingEvents = requiredEventTypes.filter(et =>
            !usedEventTypes.some(used => used.includes(et))
          );
   ...
   */
  private async validateEventTypes(): Promise<SecurityIssue[]> {
    const issues: SecurityIssue[] = [];
    const loggerPath = `${this.projectRoot}/src/lib/utils/audit-logger.ts`;
   ...
   */
  private async validateMetadataCapture(): Promise<SecurityIssue[]> {
    const issues: SecurityIssue[] = [];
    const loggerPath = `${this.projectRoot}/src/lib/utils/audit-logger.ts`;
   ...
   */
  private getSensitiveOperations(path: string, content: string): AuditableOperation[] {
    const operations: AuditableOperation[] = [];
   ...
   */
  private extractAuditEventTypes(content: string): string[] {
