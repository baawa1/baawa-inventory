...
export class PermissionAuditor {
   ...
   * Main audit function
   */
  async audit(): Promise<SkillResult<AuditReport>> {
    try {
   ...
   */
  private async loadPermissionDefinitions(): Promise<void> {
   ...
   */
  private async analyzeApiRoutes(): Promise<RouteAnalysis[]> {
   ...
   */
  private async analyzeMiddleware(): Promise<SecurityIssue[]> {
   ...
   */
  private async findHardcodedRoleLogic(): Promise<SecurityIssue[]> {
    const issues: SecurityIssue[] = [];
    const apiFiles = await FileUtils.findFiles('src/app/api/**/route.ts');
   ...
   */
  private isSensitiveRoute(path: string, content: string): boolean {
   ...
   */
  private generateReport(issues: SecurityIssue[], routes: RouteAnalysis[]): AuditReport {
