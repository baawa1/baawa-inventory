...
export class SchemaValidator {
  ...
  async validate(): Promise<SkillResult<CodeIssue[]>> {
    try {
  ...
  private extractForeignKeys(content: string): Array<{ model: string; field: string }> {
  ...
  private extractMoneyFields(content: string): Array<{ name: string; type: string }> {
  ...
  private extractRelations(content: string): Array<{ name: string; onDelete?: string; optional: boolean }> {
    const relations: Array<{ name: string; onDelete?: string; optional: boolean }> = [];
  ...
  private hasIndex(content: string, fieldName: string): boolean {
