-- Enable Row Level Security (RLS) for Auth.js authentication system
...
RETURNS INTEGER AS $$
BEGIN
  -- For Auth.js, we'll use a session-based approach
  -- This function will be called from application context
  RETURN NULL; -- Will be overridden by application logic
END;
...
RETURNS BOOLEAN AS $$
BEGIN
  -- This will be called from application context
  RETURN FALSE; -- Will be overridden by application logic
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create a function to check if user is staff or admin
...
CREATE POLICY "Users can view items from their transactions" ON sales_items
    FOR SELECT USING (
        EXISTS (
            ...
            WHERE id = sales_items.transaction_id 
...
CREATE POLICY "Users can view their own split payments" ON split_payments
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM sales_transactions 
            WHERE id = split_payments.transaction_id 
            AND user_id = get_current_user_id()
        )
    );
...
CREATE POLICY "Users can view their own reports" ON financial_reports
    FOR SELECT USING (generated_by = get_current_user_id());

CREATE POLICY "Users can create reports" ON financial_reports
    FOR INSERT WITH CHECK (generated_by = get_current_user_id());
