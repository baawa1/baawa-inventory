...
async function testPendingUsersAPI() {
  ...
  try {
    ...
    const response = await fetch("http://localhost:3000/api/users?status=pending", {
      headers: {
    ...
    const { data: sampleUsers, error } = await supabase
      .from("users")
      .select("id, first_name, last_name, email, user_status, email_verified, role, is_active, created_at")
      .limit(1);
