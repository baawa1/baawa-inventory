...
async function checkUsers() {
  try {
    ...
    const { data: users, error } = await supabase
      .from("users")
      .select(
        "id, first_name, last_name, email, role, user_status, email_verified, is_active"
      )
      .limit(5);
    ...
    users.forEach((user) => {
      console.log(
        `- ${user.first_name} ${user.last_name} (${user.email}) - Role: ${user.role}, Status: ${user.user_status}`
      );
    });
