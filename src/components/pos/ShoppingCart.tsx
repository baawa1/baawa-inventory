...
export function ShoppingCart({
  ...
  const handleSavePrice = () => {
    ...
    onUpdatePrice(editingItem.id, normalizedPrice, reasonInput.trim());

    toast.success('Price updated for cart item.');
    resetEditor();
  };
  ...
  const handleResetPrice = () => {
    ...
    onUpdatePrice(editingItem.id, editingItem.basePrice);
    toast.success('Price reset to product price.');
    resetEditor();
  };
    ...
    <div className="flex h-full min-h-0 flex-col">
      {isEmpty ? (
        <div className="text-muted-foreground flex flex-1 flex-col items-center justify-center py-8 text-center">
          <IconShoppingCartOff className="mb-2 h-12 w-12 opacity-50" />
                  ...
                  <div className="space-y-1 sm:space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        {item.priceOverride ? (
                          <>
                            <div className="text-muted-foreground text-xs line-through sm:text-sm">
                              {formatCurrency(item.basePrice)} each
                            </div>
                            <div className="text-emerald-600 text-xs font-semibold sm:text-sm">
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            {editingItem && (
              <DialogDescription>
                Adjust the price for <strong>{editingItem.name}</strong>.
              </DialogDescription>
            )}
          </DialogHeader>
            ...
            <div className="space-y-4 py-2">
              <div className="rounded bg-muted p-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span>{formatCurrency(editingItem.basePrice)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Current cart price:</span>
